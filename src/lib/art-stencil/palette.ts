...
/**
 * Palette names are local to this art set. Nothing outside it knows or cares what
 * they are — the engine only reads `name`, `tint` and `tintOpacity` (see
 * `ScenePalette` in `src/lib/story.ts`), which this satisfies structurally. Rename
 * these, delete them, invent new ones: no other story is affected.
 */
